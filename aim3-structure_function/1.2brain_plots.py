...
def assign_region_names(df, missing=False):
    ...
    fc_cort_var = series.filter(regex='rsfmri.*_c_.*').index
    fc_scor_var = series.filter(regex='rsfmri.*_cor_.*').index
    fmri_var_var = series.filter(regex='rsfmri.*_var_.*').index
...
rnd_brain_loadings = pd.read_csv(join(PROJ_DIR, OUTP_DIR, "rni-brain_P.csv"), header=0, index_col=0)
...
for sig_dim in sig_dims[:4]:
    ...
    plotting.plot_img_on_surf(
            ...
            output_file=join(PROJ_DIR, FIGS_DIR, f'rnd_{loadings.name}.png')
        )

rnd_brain_loadings.to_csv(join(PROJ_DIR, OUTP_DIR, "rnd-brain_P-region_names.csv"))
